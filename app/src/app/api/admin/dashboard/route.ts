...
export async function GET(request: NextRequest) {
  try {
    ...
    const partnerIds = pendingPartnerUsers.map((user: any) => user._id);
    ...
    const pendingPartnersReviews = pendingPartnerUsers.map((user: any) => ({
      _id: user._id,
      name: user.name,
      email: user.email,
    ...
    const pendingVehicles = await Vehicle.find({
      ...
      "owner",
      "name email mobileNumber profilePicture partnerOnBoardingSteps",
    );

    const pendingVehiclesReview = pendingVehicles.map((vehicle: any) => ({
      _id: vehicle._id,
      name: vehicle.owner?.name || "Unknown Partner",
      email: vehicle.owner?.email || "",
      mobile: vehicle.owner?.mobileNumber || "",
      profilePicture: vehicle.owner?.profilePicture || "",
